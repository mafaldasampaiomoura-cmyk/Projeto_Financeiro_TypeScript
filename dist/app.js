const botao = document.querySelector(".adiciona-historia"); //TS: HTML Button Element -- basicamente diz ao TS que é um botão, que tem as API's corretas e que evita erros em cascata. 
if (!botao) {
...
botao.addEventListener("click", () => {
    const descricao = document.querySelector("#descricao"); // Aqui faço input porque eles são inputs mesmo 
    if (!descricao) {
    ...
    const tipoValor = tipo.value; // defino aqui as variáveis que vou receber. Ou seja o meu tipo só pode ser receita ou despesa 
    const valorNumero = Number(quanTipoValor);
    if (descricaoValor.trim() === "" ||
        quanTipoValor.trim() === "" ||
        tipoValor.trim() === "") {
